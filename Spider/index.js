...
fetchNextURLs = function(url){
    request({url: url}, function (error, response, body) {
        ...
        var hrefs = [];
        $('#recommendations dl img').each(function(k,v){
            var $me = $(this);
            var href = Tools.getUrl( $me.attr('src') );
            var numbers = Tools.getNumbersOfUrl(href);
            if(!urlDic[numbers]){
                urlDic[numbers] = true;
                hrefs.push(href);
                outStream.write(href+ '\r\n');
                var htmlStream = fs.WriteStream('./douban_movies_img/img' + k + '-' + numbers + '.jpg');
                htmlStream.write(body);
                htmlStream.end();
            }
        });
